...
This reference defines the detailed operating method for step 5 in `docsraw-sync-run`.

## Why this exists

Step 5 is the architectural center of the sync workflow. It must merge two intentionally isolated analyses into one actionable plan while preserving:
...
3. **Scalability (weight 4)**  
   Will it handle growth in rawdocs volume/topics over repeated syncs?
