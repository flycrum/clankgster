...
- Keep `rawdocs/` physically intact during sync (do not rewrite, delete, or normalize `rawdocs/` files as part of output generation).
...
- Any sync or refactor step where paths are planned or links are validated
- Any lint/audit check for plugin link hygiene

## When it does not apply

- Reading `rawdocs/` contents for analysis in isolated ingestion steps
- Internal path handling for file IO during sync execution, where no markdown links are emitted
