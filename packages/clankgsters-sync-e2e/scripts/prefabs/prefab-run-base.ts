...
/**
 * Base class for prefab run (materialize) stages.
 * Provides protected fs/path helpers so subclasses avoid repeated utility imports.
 */
export abstract class PrefabRunBase<TOptions extends object = Record<string, never>> {
  constructor(
  ...
  abstract run(context: PrefabApplyContext): void;
