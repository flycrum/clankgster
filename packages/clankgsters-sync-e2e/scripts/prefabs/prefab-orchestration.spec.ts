...
import { prefabOrchestration } from './prefab-orchestration.js';
...
function createContext(caseOutputRoot: string): PrefabApplyContext {
  return {
    caseIndex: 1,
    caseName: 'unit',
    caseOutputRoot,
...
describe('prefabOrchestration', () => {
  test('throws when no explicit action is available', () => {
    expect(() =>
      prefabOrchestration.resolvePrepareConfig(
        {
          groups: [
            {
              entries: [
  ...
  test('applies replace roots before entry run', () => {
    ...
    const resolvedPrepare = prefabOrchestration.resolvePrepareConfig(
      {
        groups: [
          {
            action: 'replace',
