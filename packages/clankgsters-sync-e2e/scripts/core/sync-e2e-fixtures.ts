 ...
 * last harness run back into each `scripts/test-cases/<caseId>/` folder so committed expectations stay in
...
async function main(): Promise<void> {
  ...
  const scriptsRoot = path.resolve(coreDir, '..');
  const packageRoot = path.resolve(scriptsRoot, '..');
  const testCasesDir = e2ePathHelpers.getTestCasesRoot(scriptsRoot);
