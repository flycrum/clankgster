...
- **`clankgster.config.ts`** — `agents`, `excluded`, `sourceDefaults` (`localMarketplaceName`, `markdownContextFileName`, `sourceDir`, `pluginsDir`, `skillsDir`), `artifactMode` (`copy` default, `symlink` opt-in), `transforms` (`registry`, `hooks`, `options`, `templateVariables.{openingDelimiterToken,closingDelimiterToken}`), `syncOutputReadOnly`, `syncCacheDir`, `syncManifestPath`, `loggingEnabled`
