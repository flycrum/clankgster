import { npmPackFilesToLintGlobs } from '@clankgsters/vite-plus-config/common/npm-pack-files-to-lint-globs';
...
/** Enforced for `@clankgsters/sync` only; keep aligned with `.oxlintrc.jsonc` (editor / `pnpm exec oxlint`). */
const syncMustNotDependOnE2eRules: NonNullable<OxlintConfig['rules']> = {
  'eslint/no-restricted-imports': [
    'error',
    {
      paths: [
        {
          name: '@clankgsters/sync-e2e',
          message:
            '@clankgsters/sync must not depend on @clankgsters/sync-e2e; shared code belongs in this package.',
        },
      ],
      patterns: [
        {
          regex: 'clankgsters-sync-e2e',
          message: '@clankgsters/sync must not import paths containing clankgsters-sync-e2e.',
        },
      ],
    },
  ],
