# Pino logging in Clankgsters sync

When editing or adding code under `packages/clankgsters-sync/scripts/` (sync runner, adapters, discovery, config, etc.):
...
**Compliance:** Search for raw `console.*` under `scripts/` and fix or justify. Confirm entry points set logger context before `getLogger()`.

See [pino-logger plugin README](../README.md) for enable/disable and log path (`.clank/logs/clankgsters-sync.log`).
