...
Guidance for publishing **@clankgsters/sync** to npm. No release tool or new dependencies: use `npm version` and `npm publish` only. This plugin provides rules and a deploy wizard skill so an agent can walk the developer through version choice, dist-tag, and publish steps.
