...
`@clankgsters/sync` is the publishable sync runtime package. **`src/`** holds the library and CLI implementation; **`scripts/`** holds thin entry files that are still shipped in `files` so consumers can execute them with `tsx` or reference paths under `node_modules`.
...
## Monorepo root `clankgsters.config.ts`

- Importing **`./packages/clankgsters-sync/src/index.js`** is a **repo-relative path to TypeScript sources** resolved by the TS tooling; no `pnpm` workspace link is required.
- Adding **`@clankgsters/sync`** to the root `package.json` as a **`workspace:*`** dependency is valid and lets you write `import { … } from '@clankgsters/sync'`, but that resolves to **`dist/index.mjs`** per `package.json` `exports`, so you typically need `pnpm -F @clankgsters/sync build` before Node loads the package entry without a bundler. For repo config files that are edited in TS without a prior build, **relative imports into `src/index.ts`** stay the practical choice.
